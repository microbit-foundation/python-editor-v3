...
export const renderDocumentation = (
  ...
  if (MarkupContent.is(documentation) && documentation.kind === "markdown") {
    try {
      div.innerHTML = renderMarkdown(documentation.value, brief).__html;
      return div;
    } catch (e) {
  ...
  let fallbackContent = MarkupContent.is(documentation)
    ...
    : documentation;
  if (brief) {
    fallbackContent = splitDocString(fallbackContent).summary;
  }
...
export const renderMarkdown = (
  ...
  if (brief) {
    const parts = splitDocString(markdown);
    markdown =
      parts.summary + (parts.example ? `\n\n\`${parts.example}\`` : "");
  }
